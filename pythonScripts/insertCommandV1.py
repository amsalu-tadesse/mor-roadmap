...
def migrate_wps_to_mysql():
    ...
    try:
        ...
        for row in sheet.iter_rows(min_row=2):
            ...
            # Map values based on column order placement array layouts
            ...
            #     extra_obj = {"theme_id": current_theme_id} if current_theme_id else {}
            #     current_directorate_id = get_or_create(cursor, "directorates", directorate_raw, user_id, now_str, extra_fields=extra_obj)
            
              # Process directorate dynamically: Inserts if missing, returns unique ID
            directorate_id = None
            if directorate_raw and str(directorate_raw).strip() != "":
                directorate_id = get_or_create(cursor, "directorates", directorate_raw, user_id, now_str)
            ...
            if initiative_raw and str(initiative_raw).strip() != "":
                current_directorate_initiative_id = get_or_create_directorate_initiative(cursor, current_initiative_id, directorate_id, now_str)
                
            # ----------------------------------------------------------------
            # 4. STANDALONE TABLE UPSERTS (STATUS & DIRECTORATES)
            # ----------------------------------------------------------------
            ...
            if activity_raw and str(activity_raw).strip() != "":
                ...
                # Note: If your activities schema table doesn't have a direct directorate_id column,
                # you can move this variable link up into your initiatives query instead.
                ...
                # ----------------------------------------------------------------
                # 6. RESOLVE SPLIT AND MULTI-PARTNER ASSIGNMENTS (PIVOT ENGINE)
                # ----------------------------------------------------------------
        ...
        # ----------------------------------------------------------------
        # 7. CLEAN LEADING NUMBERS AND DOTS FROM INITIATIVES TABLE
        # ----------------------------------------------------------------
        print("Cleaning up leading numbers and dots from initiatives...")
        
        # This SQL query matches text starting with optional whitespace, 
        # numbers, dots, more numbers/dots, and trims off any extra spaces.
        ...
        # ----------------------------------------------------------------
        # 8. BACKFILL FIRST/LATEST PARTNER ID INTO THE ACTIVITIES TABLE
        # ----------------------------------------------------------------
        print("Backfilling partner reference links into the activities table...")
        
        # This query assigns a sequential rank to partners mapped to the same activity, 
        # grabs the target record, and updates the core activities table.
        # Change 'ORDER BY id ASC' to 'ORDER BY id DESC' if you want the absolute latest.
