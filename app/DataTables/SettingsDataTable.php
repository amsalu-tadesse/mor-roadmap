...
class SettingsDataTable extends DataTable
{
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        ...
        return (new EloquentDataTable($query))
            ...
            ->addColumn('value1', function ($setting) {
                if ($setting->value1 == 1 && $setting->type == 0) {
                    return '<span class="badge badge-success">On</span>';
                }
                if ($setting->value1 == 0 && $setting->type == 0) {
            ...
            ->addColumn('action', function ($setting) {
                return view('components.action-buttons-for-settings', [
                    'row_id' => $setting->id,
                    'permission_delete' => 'setting: delete',
                    'permission_edit' => 'setting: edit',
                    'permission_view' => 'setting: view',
                ]);
            })
            ->rawColumns(['no', 'value1', 'action']);
    }
