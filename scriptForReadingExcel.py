...
def save_to_db():
    ...
    tables_to_truncate = ['partners', 'request_statuses', 'initiative_statuses', 'themes', 'directorates']

    with engine.connect() as conn:
        # 1. Turn off foreign key constraint checks
        conn.execute(text("SET FOREIGN_KEY_CHECKS = 0;"))

        # 2. Run your truncation loop
    ...
    try:
        ex_init = pd.read_sql('SELECT name, objective_id, directorate_id FROM initiatives', con=engine)
        final_initiatives = final_initiatives[~final_initiatives.set_index(['name', 'objective_id', 'directorate_id']).index.isin(
            ex_init.set_index(['name', 'objective_id', 'directorate_id']).index
        )]
    except: pass
    ...
    if COL_ACT in df.columns:
        ...
        print("Truncating support_requests for fresh sync...")
        with engine.connect() as conn:
            conn.execute(text("SET FOREIGN_KEY_CHECKS = 0;"))
            conn.execute(text("TRUNCATE TABLE support_requests;"))
            conn.execute(text("SET FOREIGN_KEY_CHECKS = 1;"))
            conn.commit()
